...
| Sha of Pride | Mark of Arrogance x Gift of the Titans | **B**: HOLDING the dispel until Gift of the Titans (144359, ~25.5s cadence, healer-targeted) is CORRECT play — dispelling Mark costs Pride; under Gift it is free. End-of-fight burn = free dispels too. A "slow dispel table" here is a POSITIVE finding. **Measurable KPI**: Mark of Arrogance *damage-taken* DTPS (id 144351) per pull = the COST of leaving stacks up; it collapses on a clean kill (measured 60k DTPS on the kill vs 138-285k on the wipes) → use it as the dispel-discipline signal, not a raw dispel count | timestamp correlation dispels x 144359 windows; per-pull 144351 DTPS (taken/duration) on the live run |
...
| General Nazgrim | Defensive Stance (rage) | The "who fed rage in Defensive" table must count **player-attributed damage ONLY**. EXCLUDE (harmless, no rage): DoTs applied before the stance (ticks), cleave on the Banner, AND **autonomous pet/guardian damage** (auto-attack, Water Elemental, Dire Beast, ghoul, Gargoyle, GoAK, Dancing Rune Weapon). RE-INCLUDE the rage-adding exceptions to the owner: **Kill Command** (pet ability 83381 = player-activated) and **DoT applications** (`tick=0`, not ticks). Rage is fed by player-attributed damage only — counting pet swings falsely blames hunters/locks/DKs. **v2:** also EXEMPT the **tank holding Sundering Blow** (`Coup destructeur` 143494 — the ONLY tooltip-written exclusion; per-interval, handles swaps; tanks otherwise dominate, 58M→4M generating); EXCLUDE **autonomous procs attributed to the player** (curated list: legendary cloaks 148008/147891/149276, meta-gem Foudre 137597/141004/138146, Stormlash 120687, mastery incl. **Hand of Light 96172 = mastery proc, NOT a DoT** + Icicle 148022, Seal 42463/Poison 113780/Lightning Shield 26364, Shadow apparitions 148859+73510); **gate cast-then-pecks** (Corbeaux dmg 131900 counts only if cast 131894 fell during the stance). **Provenance caveat:** pet/proc exemptions are PLAYER-TESTED + tooltip, NOT WCL-measurable (rage energize logs source=boss). Names: use the Wowhead **mop-classic** tooltip endpoint (retail returns empty for old base ids → "#xxxx"). | per-player cast/DoT timeline vs stance windows; pet/proc exemption = player tests + tooltip, NOT WCL-measured. Live run: ~50M autonomous pet/guardian dmg, all rage-irrelevant; tanks ~88M under Sundering exempted; only Kill Command (2.3M) a real owner gain |
...
| Fallen Protectors HM | Inferno Strike (143962) | **D**: SHARED damage (~2.8M total 25H) split among players within range of the target — a death here is a SOAK-HEADCOUNT failure (collective), never an individual blame. Measured: n=10 soakers → 287k each (survivable), n=3 → 895k each (one-shot). First cast lands at 41-47 s every pull. Kill norm = assigned squads of 5-6 | per-cast target grouping (500 ms window) on deep_dmg_taken, 12 pulls 25H |
...
| Sha of Pride HM | Rift of Corruption (soak) | **HEROIC-only** mechanic (0 in N — the Normal kill has zero soak hits, which is literally why dropping difficulty removes it). The rift (a `Rift of Corruption` NPC) pulses **Unstable Corruption** 147198 = 350k Shadow within 2 y **AND +5 Pride to each player hit**, and on close explodes **Rift Collapse** 147388 = 250k Shadow within 8 y. Soaking = eating big hits AND injecting Pride (the fuel for the Swelling Pride pulses ~78 s) → the problem is **soaker rotation + Pride economy**, NOT just healing. "Pride from rifts" = hits × 5 = a COMPONENT proxy, NEVER the full Pride bar (the bar isn't logged). A high-soak player = assigned OR mispositioned — the log can't tell; frame collectively, never "X never soaked". Soak load IS measurable here (147198 is a damage event, unlike Norushen orbs) | per-pull 147198 hits vs distinct soakers + collapse 147388 count, source = Rift of Corruption NPC; Normal-kill witness = 0 hits; IDs verified wowhead mop-classic |
| Sha of Pride HM | Corrupted Prison / Banishment (time locked) | Time spent locked away = a break-SPEED metric, not a fault on the prisoner (the RAID frees them by breaking the prison). Measure per pull by pairing `applydebuff`→`removedebuff` per (target, ability): **Corrupted Prison** = 5 debuff ids 144574/144615/144636/144683/144684 (cast = Imprison 144563; ~4-8 prisoners/pull, freed in ~4-7 s avg, fastest on the kill); **Banishment** 145215 (HM realm lockout). Lower avg = freed faster = better. Witness: Banishment avg = 0 in Normal (HM-only). | apply→remove pairing on deep_aura per (target,ability), 6 HM pulls + N kill |
